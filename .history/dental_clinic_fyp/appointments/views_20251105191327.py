...
def dashboard_doctor(request):
    ...
    upcoming_appointments = appointments.filter(
        status__in=["Pending", "Confirmed", "Reschedule Requested", "Reschedule Approved"]
    ).order_by("date")

    completed_appointments = appointments.filter(status="Completed").order_by("-date")
    cancelled_appointments = appointments.filter(status="Cancelled").order_by("-date")
    ...
    return render(request, "appointments/report_form.html", context)
