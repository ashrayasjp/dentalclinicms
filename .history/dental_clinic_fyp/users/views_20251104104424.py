...
def register_view(request):
    if request.method == 'POST':
        ...
        if not re.match(pw_regex, password1):
            messages.error(request, "Weak password: min 6 chars, uppercase, lowercase, number, special character.")
            return render(request, "users/register.html", {'form_data': form_data})
        ...
        if role == "doctor":
            # Identity document is required for doctors
...
from django.contrib.auth.decorators import user_passes_test
from django.contrib.auth import get_user_model
from .models import Doctor, DoctorRegistrationRequest
