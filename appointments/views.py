...
def doctor_details(request, doctor_id):
    ...
    data = {
        ...
        'degree': doctor.degree or 'N/A',
        'specialization': doctor.specialization or 'N/A',
        'experience_years': doctor.experience_years or 0,
        'bio': doctor.bio or 'N/A',
        'profile_picture': doctor.profile_picture.url if doctor.profile_picture else '/static/images/default_profile.png'
    }
...
from django.shortcuts import render, redirect, get_object_or_404
from django.contrib import messages
from .models import Appointment, Report, Patient
...
def appointment_list(request):
    ...
    # --- Auto-cancel overdue appointments (24+ hours passed & no report exists) ---
    ...
    for appt in overdue_appointments:
        if not Report.objects.filter(appointment=appt).exists():
            appt.status = "Cancelled"
            appt.save()

    # --- Auto-cancel reschedule rejected appointments ---
    ...
    # --- Prevent marking appointments as completed without a report ---
    completed_without_report = Appointment.objects.filter(
        patient=patient,
        status="Completed"
    )
    for appt in completed_without_report:
        if not Report.objects.filter(appointment=appt).exists():
            appt.status = "Pending"
            appt.save()
...
def reschedule_request(request, appointment_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            appointment.save()
            messages.success(request, "Reschedule request sent. Waiting for admin's approval.")
            return redirect('appointments:appointment_list')
...
def doctors_view(request):
    doctors = Doctor.objects.all().values('id', 'full_name', 'degree', 'specialization', 'experience_years', 'bio', 'profile_picture')
    doctors_json = json.dumps(list(doctors))
    return render(request, 'your_template.html', {'doctors_json': doctors_json})
