...
def register_view(request):
    if request.method == 'POST':
        ...
        form_data = request.POST.copy()  # to repopulate form if error occurs
        ...
        if User.objects.filter(username=username).exists() or DoctorRegistrationRequest.objects.filter(username=username).exists():
            messages.error(request, "Username already exists.")
            return render(request, "users/register.html", {'form_data': form_data})

        if User.objects.filter(email=email).exists() or DoctorRegistrationRequest.objects.filter(email=email).exists():
        ...
        if role == "doctor":
            ...
            # ✅ Create DoctorRegistrationRequest (not a Patient)
            DoctorRegistrationRequest.objects.create(
                ...
                password=password1,  # stored temporarily for admin approval
                degree=degree,
            ...
            messages.success(request, "Your doctor registration request has been submitted for admin approval.")
